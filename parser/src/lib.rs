...
pub const KIND_TELEMETRY: u8 = 0x01;
...
// читает ровно один кадр из потока. границы кадра известны из заголовка, так что
// внешний length-prefix не нужен
pub fn read_frame<R: Read>(r: &mut R) -> Result<Vec<u8>, ReadErr> {
    let mut magic = [0u8; 2];
...
// parse_frame разбирает один бинарный кадр в строку json.
// используется и из stdin-бинаря, и из NIF-обёртки.
pub fn parse_frame(frame: &[u8]) -> Result<String, String> {
...
// ---- Rustler NIF ----
// DirtyCpu: разбор может быть тяжёлым на больших кадрах — не блокируем обычные
// планировщики BEAM, пускаем на dirty-планировщик.
// Вход — Elixir binary (rustler::Binary), не Vec<u8> (Vec декодируется из списка).
#[rustler::nif(schedule = "DirtyCpu")]
fn parse<'a>(binary: Binary<'a>) -> Result<String, String> {
    parse_frame(binary.as_slice())
}

// rustler 0.38: список NIF-функций собирается автоматически через атрибут
// #[rustler::nif], здесь только имя Elixir-модуля.
rustler::init!("Elixir.Gateway.Parser");
