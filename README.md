...
Мобилка:
- **Android**: ARCore через SceneView (`io.github.sceneview:arsceneview:2.3.0`) —
  камера + SLAM-трекинг плоскостей + тепловая карта (`XRayARView.kt`);
- **iOS**: ARKit (`ARSCNView` + детекция плоскостей + тепловая карта) —
  `XRayARView.swift`.

Тепловая карта: холод = синий, тепло = красный. Привязка 3D-труб к SLAM-якорям —
доработка под конкретную схему дома (TODO в коде). Ультразвуковая эхо-локализация —
вне объёма (нужен DSP).

> **Примечание**: `minSdk` Android поднят до 26 (требование Health Connect/ARCore).
> HRV читается реально: HealthKit (iOS) и Health Connect (Android, `HeartRateVariabilityRmssd`).
